...
function greetUser(name) {
  ...
  return `Welcome to brutalist design, ${name}`;
}
...
```bash
#!/bin/bash
# Deploy script for brutalist website
...
func (s *Server) handleRoot(w http.ResponseWriter, r *http.Request) {
    fmt.Fprintf(w, "Welcome to the Brutalist Design API!")
}

func (s *Server) handleHealth(w http.ResponseWriter, r *http.Request) {
    w.Header().Set("Content-Type", "application/json")
    fmt.Fprintf(w, `{"status": "healthy", "theme": "brutalist"}`)
}
...
```json
{
  "name": "brutalist-design-blog",
  "version": "1.0.0",
  "description": "A blog with brutalist design aesthetic",
  "scripts": {
    "dev": "next dev",
    "build": "next build",
    "start": "next start",
